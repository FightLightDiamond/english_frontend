<template>
  <div class="sidebar" @mouseenter="isMenuOver=true" @mouseleave="isMenuOver=false" @touchstart="isMenuOver=true">
    <div class="main-menu">
      <vue-perfect-scrollbar class="scroll" :settings="{ suppressScrollX: true, wheelPropagation: false }">
        <ul class="list-unstyled">
          ...
          <li :class="{ active : selectedParentMenu==='english' }">
            <a @click.prevent="openSubMenu($event,'english')" href="#english">
          ...
          <li :class="{ active : selectedParentMenu==='contact' }">
            <a @click.prevent="openSubMenu($event,'contact')" href="#contact">
              <i class="iconsminds-bird-delivering-letter"></i>
              {{$t('Contact') }}
            </a>
          </li>
    ...
    <div class="sub-menu">
      <vue-perfect-scrollbar class="scroll" :settings="{ suppressScrollX: true, wheelPropagation: false }">
        <ul class="list-unstyled" data-link="user" :class="{'d-block':selectedParentMenu==='user' }">
          <router-link tag="li" to="/administrator/users">
            <a><i class="iconsminds-user"></i> <span>{{ $t('User') }}</span></a>
          </router-link>
          <router-link tag="li" to="/administrator/admins">
            <a><i class="iconsminds-administrator"></i> <span>{{ $t('Admin') }}</span></a>
          </router-link>
        </ul>

        <ul class="list-unstyled" data-link="english" :class="{'d-block':selectedParentMenu==='english' }">
          <router-link tag="li" to="/administrator/courses">
            <a><i class="iconsminds-books"></i> <span>{{ $t('Course') }}</span></a>
          </router-link>
          <router-link tag="li" to="/administrator/lessons">
            <a><i class="iconsminds-notepad"></i> <span>{{ $t('Lesson') }}</span></a>
          </router-link>
        </ul>

        <ul class="list-unstyled" data-link="contact" :class="{'d-block':selectedParentMenu==='contact' }">
          <router-link tag="li" to="/administrator/contacts">
            <a><i class="iconsminds-communication-tower-2"></i> <span>{{ $t('Contact') }}</span></a>
          </router-link>
        </ul>
