...
<!--        {{currentUser}}-->
<!--        {{otherUser}}-->
        <conversation-detail v-if="conversationMessages" key="conversation"
...
<script>
  ...
  // require( 'http://localhost:60001/socket.io/socket.io.js' )();
  ...
  export default {
    ...
    methods: {
      ...
      async selectContact (userId) {
        ...
        // if (conversation) {
          ...
          this.conversationMessages = this.conversations
        // } else {
      ...
      sendMessage () {
        window.axios.post('/conversation/send', {
    ...
    mounted () {
      Echo.channel(`private-messages.26`)
        .listen('.NewMessage', (e) => {
          alert(12333)
